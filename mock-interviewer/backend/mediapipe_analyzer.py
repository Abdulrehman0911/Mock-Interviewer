...
def _ensure_readable_video(video_path: str):
    """Return a video path OpenCV can read on Windows.

    OpenCV-Windows often cannot decode WebM/VP9 directly. Convert to a raw
    AVI (MJPEG) that OpenCV handles natively. Returns (path, was_converted).
    """
    # Try opening directly first — if it works, no conversion needed
    ...
    # Convert via ffmpeg to MJPEG AVI (OpenCV reads this on any platform)
    tmp_avi = tempfile.NamedTemporaryFile(delete=False, suffix=".avi").name
    try:
        ...
        if result.returncode == 0 and os.path.exists(tmp_avi) and os.path.getsize(tmp_avi) > 100:
            print(f"[Analyzer] Converted {os.path.basename(video_path)} -> MJPEG AVI for OpenCV")
            return tmp_avi, True
        else:
            stderr = result.stderr.decode("utf-8", errors="ignore")
            print(f"[Analyzer] ffmpeg conversion failed: {stderr[-300:]}")
            if os.path.exists(tmp_avi):
                os.remove(tmp_avi)
            return video_path, False
    except Exception as e:
        print(f"[Analyzer] ffmpeg conversion error: {e}")
        if os.path.exists(tmp_avi):
            os.remove(tmp_avi)
        return video_path, False
...
def analyze_video_behavior(video_path: str, target_fps: int = 2) -> Dict[str, object]:
    ...
    # Ensure OpenCV can read the file (converts WebM/VP9 to MJPEG AVI if needed)
    readable_path, was_converted = _ensure_readable_video(video_path)
    _cleanup_converted = readable_path if was_converted else None
    ...
    try:
        ...
        if not cap.isOpened():
            cap.release()
            print(f"[Analyzer] ERROR: Cannot open video: {readable_path}")
            if _cleanup_converted:
                try: os.remove(_cleanup_converted)
